...
rbf_kernal = svm.SVC(kernel='rbf', gamma=0.1, C=1.0)
...
predictions = rbf_kernal.predict(X_test)

accuracy_rbf = accuracy_score(y_test, predictions)
precision_rbf = precision_score(y_test, predictions, average='weighted')
recall_rbf = recall_score(y_test, predictions, average='weighted')
f1_rbf = f1_score(y_test, predictions, average='weighted')


#Guarda el modelo entrenado y las métricas en archivos separados usando joblib
joblib.dump(rbf_kernal, 'svm_model_rbf.joblib')
...
joblib.dump(metrics_rbf, 'svm_metrics_rbf.joblib')

#Guardar los vectores de soporte
support_vectors = rbf_kernal.support_vectors_

#Guardar los vectores de soporte en un archivo separado usando joblib
joblib.dump(support_vectors, 'svm_support_vectors_rbf.joblib')


#Metricas de evaluacion 

#Calcular y mostrar la precision del modelo en el conjunto de prueba
accuracy = rbf_kernal.score(X_test, y_test)
...
#Guardar Plot
plt.savefig('matriz_rbf.png')
plt.show()

#Archivo matriz de confusion
joblib.dump(conf_matrix, 'confusion_matrix_rbf.joblib')

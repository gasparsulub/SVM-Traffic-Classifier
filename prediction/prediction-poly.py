...
poly_kernel = joblib.load('svm_model_poly.joblib')  
support_vectors = joblib.load('svm_support_vectors_poly.joblib')  
...
predictions_weighted = poly_kernel.predict(df_predict.drop("Label", axis=1).values)
...
joblib.dump(metrics_data, 'predictions_metrics_poly.joblib')

# Guardar la matriz de confusión en un archivo Joblib
joblib.dump(conf_matrix_weighted_df, 'prediction_matrix_poly.joblib')
...
df_predict.to_csv('predictions_poly.csv', index=False)

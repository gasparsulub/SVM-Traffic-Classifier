...
linear_kernel = joblib.load('svm_model_linear.joblib')  
support_vectors = joblib.load('svm_support_vectors_linear.joblib')  
...
predictions_weighted = linear_kernel.predict(df_predict.drop("Label", axis=1).values)
...
joblib.dump(metrics_data, 'predictions_metrics_linear.joblib')

# Guardar la matriz de confusión en un archivo Joblib
joblib.dump(conf_matrix_weighted_df, 'prediction_matrix_linear.joblib')
...
df_predict.to_csv('predictions_linear.csv', index=False)

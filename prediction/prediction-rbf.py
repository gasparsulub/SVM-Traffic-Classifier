...
rbf_kernel = joblib.load('svm_model_rbf.joblib')  
support_vectors = joblib.load('svm_support_vectors_rbf.joblib')  
...
predictions_weighted = rbf_kernel.predict(df_predict.drop("Label", axis=1).values)
...
joblib.dump(metrics_data, 'predictions_metrics_rbf.joblib')

# Guardar la matriz de confusión en un archivo Joblib
joblib.dump(conf_matrix_weighted_df, 'prediction_matrix_rbf.joblib')
...
df_predict.to_csv('predictions_rbf.csv', index=False)

...
poly_kernal = svm.SVC(kernel='poly', degree=1, gamma='auto', C=1.0)
...
predictions = poly_kernal.predict(X_test)

accuracy_poly = accuracy_score(y_test, predictions)
precision_poly = precision_score(y_test, predictions, average='weighted')
recall_poly = recall_score(y_test, predictions, average='weighted')
f1_poly = f1_score(y_test, predictions, average='weighted')


#Guarda el modelo entrenado y las métricas en archivos separados usando joblib
joblib.dump(poly_kernal, 'svm_model_poly.joblib')
...
joblib.dump(metrics_poly, 'svm_metrics_poly.joblib')

#Guardar los vectores de soporte
support_vectors = poly_kernal.support_vectors_

#Guardar los vectores de soporte en un archivo separado usando joblib
joblib.dump(support_vectors, 'svm_support_vectors_poly.joblib')


#Metricas de evaluacion 

#Calcular y mostrar la precision del modelo en el conjunto de prueba
accuracy = poly_kernal.score(X_test, y_test)
...
#Guardar Plot
plt.savefig('matriz_poly.png')
plt.show()

#Archivo matriz de confusion
joblib.dump(conf_matrix, 'confusion_matrix_poly.joblib')

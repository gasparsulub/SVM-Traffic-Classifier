...
sigmoid_kernal = svm.SVC(kernel='sigmoid', gamma='auto', coef0=0.0, C=1.0)

sigmoid_kernal.fit(X_train, y_train)
...
predictions = sigmoid_kernal.predict(X_test)

accuracy_sigmoid = accuracy_score(y_test, predictions)
precision_sigmoid = precision_score(y_test, predictions, average='weighted')
recall_sigmoid = recall_score(y_test, predictions, average='weighted')
f1_sigmoid = f1_score(y_test, predictions, average='weighted')


#Guarda el modelo entrenado y las métricas en archivos separados usando joblib
joblib.dump(sigmoid_kernal, 'svm_model_sigmoid.joblib')
...
joblib.dump(metrics_sigmoid, 'svm_metrics_sigmoid.joblib')

#Guardar los vectores de soporte
support_vectors = sigmoid_kernal.support_vectors_

#Guardar los vectores de soporte en un archivo separado usando joblib
joblib.dump(support_vectors, 'svm_support_vectors_sigmoid.joblib')


#Metricas de evaluacion 

#Calcular y mostrar la precision del modelo en el conjunto de prueba
accuracy = sigmoid_kernal.score(X_test, y_test)
...
#Guardar Plot
plt.savefig('matriz_sigmoid.png')
plt.show()

#Archivo matriz de confusion
joblib.dump(conf_matrix, 'confusion_matrix_sigmoid.joblib')
